...
const MemoryChart: React.FC<MemoryChartProps> = ({ data }) => {
  ...
  const usedMemoryPercent = data.map((d) => {
    const used = Number(d.memory.usedGB);
    const total = Number(d.memory.totalGB);
  ...
  const options: ApexOptions = {
    ...
    xaxis: {
      categories: labels,
      labels: { show: false, rotate: -45, style: { colors: "#6b7280" } },
    ...
    yaxis: {
      ...
      title: { text: "Memory Load (%)", style: { color: "#374151", fontWeight: 600 } },
    },
